...
def coin_check(bot, update):
    ...
    #coins1 = db_session.query(UserQuery).filter(UserQuery.user_coin_name==CoinBase.id).all()
    #coins2 = db_session.query(UserQuery, CoinBase).filter(UserQuery.user_coin_name==CoinBase.id).all()

    base_chat_id = db_session.query(User)
    for user_chat_id1 in base_chat_id:
        user_chat_id_dict = user_chat_id1.__dict__['id_user_chat']
        print(user_chat_id1.__dict__['id_user_chat'])
        #bot.send_message(chat_id=user_chat_id_dict, text="^_^")
        ...
        if len(coins) > 0:
            ...
            #update.message.reply_text(coin_id)
...
def query_bot(bot, update):
    ...
    try:
        ...
        print(err)
        update.message.reply_text("Something wrong with Coin!")
    ...
    try:
        ...
        elif coin_minmax == "меньше": #or coin_minmax != "Меньше":
            coin_minmax = coin_minmax

        else:
            update.message.reply_text("Something wrong with 'More or Less!'")
            return
    except IndexError as err:
        print(err)
        update.message.reply_text("1Something wrong with 'More or Less!'")
    ...
    try:
        ...
        if is_float(coin_price):
            ...
            update.message.reply_text("Something wrong with Price")
            return
    except IndexError as err:
        print(err)
        update.message.reply_text("Something wrong with Price")
    try:
        ...
        print(err)
        update.message.reply_text("Something went wrong! Are you high?")
    print(coin)
...
def main():
    ...
    #updtr.dispatcher.add_handler(CommandHandler("start1", keyboard_test)) 
